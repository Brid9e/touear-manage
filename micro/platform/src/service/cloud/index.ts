...
// 获取云端域名
let cloudUrl = localStorage.getItem('configsManage')
  ? (JSON.parse(localStorage.getItem('configsManage') as any) as any).cloudUrl +
    '/cloud-api'
  : null
if (!cloudUrl) {
...
export class Cloud {
  ...
  // 云端支付方式-不分页
  static getPayList = (params?: any, feedbackType?:string): Promise<IRes> => {
    return service.request({
      url: cloudUrl + '/configData/tree',
      method: 'get',
      params: params
    }, feedbackType)
  }
  ...
  // 查询应用所绑定的方案
  static getAppSchedule = (params: any, feedbackType?:string): Promise<IRes> => {
    return service.request({
      url: cloudUrl + '/baseScheduleBindInfo/listScheduleBindInfo',
      method: 'get',
      params: params
    }, feedbackType)
  }
  // 获取云端主题
  static getStoreTheme = ( params: any, feedbackType?:string ): Promise<IRes> => {
    return service.request({
      url: cloudUrl + '/theme/getThemes',
      method: 'get',
      params
    }, feedbackType)
  }
