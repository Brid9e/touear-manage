...
.limit-com(v-else)
  ...
  el-form-item.margin-bottom-0(v-if="limitsType === 'sno' && !openLimit", label="可见人员：")
    ...
    el-table(ref="multipleTableRef",:data="findPersonList", style="width: 100%",@select-all="handleSelecAll",  @select="handleSelect" row-key="id",)
      ...
      el-table-column(property="avatar", label="照片", show-overflow-tooltip="")
  ...
  el-form-item.margin-bottom-0(v-if="limitsType === 'rule' && !openLimit", label="添加规则：")
    RuleForm(ref="ruleFormRef", :id="appBh" :ruleData="ruleData")
</template>
...
import RuleForm from "./appEdit/ruleForm.vue"
import { isEmpty } from "lodash"

const props = defineProps(["appBh", "showResult", "value"])
const emits = defineEmits(["changeLimit", "lastDataLimit"])
const appBh = ref(props.appBh)
...
const getLimitStatus = async () => {
  const params = {
    type: "app",
    resourceId: appBh.value,
  }
  console.log("getLimitStatus------", params)
  const res = await Group.getExtendInfoSwitch(params)
  if (res.code === 200) {
    openLimit.value = res.data
    if (!res.data) {
      // false 关闭
      const resInfo = await Group.getListAfterConvertedExtendInfoWithDetail(params)
      if (resInfo.code === 200 && resInfo.data) {
        ...
        if (resInfo.data[0]?.group.length === 0 && resInfo.data[0]?.sno.length === 0) {
...
const getExtendInfo = async () => {
  const params = {
    type: "app",
    resourceId: props.appBh,
  }
  console.log("getExtendInfo------", params, props)
