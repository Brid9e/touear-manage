<!--
 * @LastEditTime: 2024-03-21 10:11:43
-->
<template lang="pug">
#channelMerchant
  HeadOperation
    el-input(:prefix-icon='Search', v-model="searchForm.name", placeholder='通过身份名称查询', clearable, @keydown.enter='getTableData("messageBox")')
    el-input(:prefix-icon='Search', v-model="searchForm.code", placeholder='通过身份代码查询', clearable, @keydown.enter='getTableData("messageBox")')
    el-button(:icon='Search', type='primary', @click='getTableData("messageBox")') 查询
    el-button(:icon='Refresh', @click='toReset') 重置
    template(#right)
      el-button(:loading="synchLoading" :icon="Refresh" type='primary', @click="onSynch" :disabled="!state.funcButton.identity_syn") 数据同步
  el-table(:data="table.tableData" v-loading="table.tableLoading" row-key='index')
    el-table-column(label='身份代码', prop="code" )
    el-table-column(label='身份名称', prop="name")
...
import { Identity } from '@/service/manage'
...
const getTableData = async (msgBox?: string) => {
  ...
  const {
    ...
    msg,
    code,
  }: any = await Identity.getBaseIdentityPage(params, msgBox)
...
//数据同步
const onSynch = () => {
  ElMessageBox.confirm('确定进行身份数据同步', '提示', {
    ...
    .then(async () => {
      ...
      const {
        ...
        code,
        msg,
      }: any = await Identity.getBaseIdentityRefres(params, 'messageBox')
      if (code === 200) {
        addSyncItem({
          method: 'post',
