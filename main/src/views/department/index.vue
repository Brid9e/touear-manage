...
.department-manage
  HeadOperation
    el-input(:prefix-icon='Search', v-model="searchForm.name", placeholder='通过部门名称查询', clearable, @keydown.enter='getTableData("messageBox")')
    el-input(:prefix-icon='Search', v-model="searchForm.code", placeholder='通过部门代码查询', clearable, @keydown.enter='getTableData("messageBox")')
    el-button(:icon='Search', type='primary', @click='getTableData("messageBox")') 查询
    el-button(:icon='Refresh', @click='toReset') 重置
    template(#right)
      el-button(:loading="synchLoading" :icon="Refresh" type='primary', @click="onSynch" :disabled="!state.funcButton.departmentSync") 数据同步
...
import { Campus, Department } from '@/service/manage'
...
const getTableData = async (msgBox?: string) => {
  ...
  const {
    ...
    msg,
    code,
  }: any = await Department.getDepartmentList(params, msgBox)
...
//数据同步
const onSynch = () => {
  ElMessageBox.confirm('确定进行部门数据同步', '提示', {
    ...
    .then(async () => {
      const {
        ...
        code,
        msg,
      }: any = await Department.syncDepartment({}, 'messageBox')
      if (code === 200) {
        addSyncItem({
          method: 'post',
