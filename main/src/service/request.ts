...
class Service {
  ...
  private errorHandle(res: any, feedbackType: string) {
    ...
    switch (res.status) {
      case 401:
        ...
        //       ? window.location.origin + '/manage/login?msg=' + res.data.message
  ...
  public request(options: AxiosRequestConfig, feedback?: string): Promise<any> {
    ...
    const optUrl: any = (options.url = (options.url as any).replace(
      '-new/',
      '/',
    ))
    if (options.url?.slice(0, 1) !== '/' && !/^https?:\/\//.test(optUrl))
